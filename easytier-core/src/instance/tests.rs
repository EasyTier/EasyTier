...
mod portable_runtime {
    ...
    async fn process_management_rpc_resolves_and_calls_core_instance_directly() {
        ...
        let manager = Arc::new(InstanceManager::new(ManagementTestFactory));
...
"#,
        ...
        let rpc = InstanceManagementRpc::<ManagementTestFactory, TestHost>::new(manager);
    ...
    async fn cancelled_delete_finishes_stop_and_keeps_wait_blocked() {
        ...
        use crate::{
            ...
            management::{
                ...
                ManagedInstanceSet, ProcessManagementRpc,
            },
        };
        use easytier_proto::{
        ...
        let instances = Arc::new(ManagedInstanceSet::new(
            BlockingFactory {
        ...
        let rpc = ProcessManagementRpc::<BlockingFactory, TestHost>::new(
            instances.clone(),
            hooks.clone(),
            storage.clone(),
        );
    ...
    async fn process_management_rpc_owns_instance_create_list_and_delete() {
        use crate::{
            config::toml::TomlConfig,
            instance::manager::InstanceFactory,
            management::{ManagedInstanceSet, ProcessManagementRpc, UnsupportedConfigFileStorage},
        };
        use easytier_proto::{
            api::manage::{
        ...
        let rpc = ProcessManagementRpc::<ManagementTestFactory, TestHost>::new(
            instances.clone(),
            Arc::new(()),
    ...
    async fn owned_selection_and_cleanup_share_the_canonical_transaction() {
        use crate::{
            ...
            instance::manager::InstanceFactory,
            management::{
                ConfigFileControl, InstanceMutationHooks, ManagedInstanceSet, ProcessManagement,
        ...
        let management = ProcessManagement::<ManagementTestFactory, TestHost>::new(
            instances.clone(),
            hooks.clone(),
            Arc::new(UnsupportedConfigFileStorage),
        );
        ...
        instances
            .run_network_instance(old_config, false, ConfigFileControl::STATIC_CONFIG)
            .unwrap();
        ...
        instances
            .run_network_instance(new_config, false, ConfigFileControl::STATIC_CONFIG)
            .unwrap();
        ...
        assert_eq!(
            instances.get_instance_name(&instance_id).as_deref(),
    ...
    async fn runtime_core_instance_owns_connectivity_lifecycle() {
        ...
        assert!(instance.packet_proxy.is_started());
