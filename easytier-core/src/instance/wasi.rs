//! Serialized lifecycle ABI and composition for a host-driven WASI core instance.
...
mod abi {
    ...
    use super::{WasiCoreInstanceCreateConfig, WasiCoreRuntime, new_wasi_core_runtime};
    ...
    pub extern "C" fn easytier_buffer_alloc(length: u32) -> u32 {
        ...
        if pointer == 0 {
            INSTANCES
                .with_borrow_mut(|registry| registry.set_error("guest buffer allocation failed"));
            return 0;
        }
