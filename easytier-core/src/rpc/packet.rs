...
pub fn preferred_compression_algo() -> CompressionAlgoPb {
...
pub async fn compress_packet(
    ...
    let algo = CompressorAlgo::try_from(accepted_compression_algo)
        .ok()
        .filter(|algo| algo.is_available())
        .unwrap_or(CompressorAlgo::None);
    let compressed = crate::packet::compressor::DefaultCompressor::new()
...
mod tests {
    ...
    fn preferred_compression_matches_build_capabilities() {
        #[cfg(feature = "zstd")]
        assert_eq!(preferred_compression_algo(), CompressionAlgoPb::Zstd);

        #[cfg(not(feature = "zstd"))]
        assert_eq!(preferred_compression_algo(), CompressionAlgoPb::None);
    }
    ...
    async fn compression_negotiation_falls_back_when_zstd_is_unavailable() {
        ...
        assert_eq!(content, b"rpc body");
        assert_eq!(algorithm, CompressionAlgoPb::None);
    }
}
