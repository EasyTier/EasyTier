...
use easytier_core::gateway::vpn_portal::{VpnPortalEvent, VpnPortalEventSink};
#[cfg(feature = "smoltcp")]
...
#[cfg(any(feature = "management", feature = "smoltcp", feature = "vpn-portal"))]
use crate::common::global_ctx::GlobalCtxEvent;
#[cfg(feature = "proxy-cidr-monitor")]
use crate::instance::proxy_cidrs_monitor::runtime_proxy_cidr_monitor_host;
...
use crate::{
    ...
    instance::config::{runtime_core_host_config, runtime_peer_manager_host_adapters},
...
#[cfg(feature = "vpn-portal")]
impl VpnPortalEventSink for GlobalCtx {
...
#[cfg(feature = "smoltcp")]
impl GatewayEventSink for GlobalCtx {
    fn emit(&self, event: GatewayEvent) {
        match event {
            GatewayEvent::PortForwardAdded(config) => {
                self.issue_event(GlobalCtxEvent::PortForwardAdded(config.into()))
            }
        }
    }
}
...
#[cfg(feature = "management")]
impl ManualConnectivityEventSink for GlobalCtx {
    fn emit(&self, event: ManualConnectivityEvent) {
        match event {
            ManualConnectivityEvent::Connecting { url } => {
                self.issue_event(GlobalCtxEvent::Connecting(url));
            }
            ManualConnectivityEvent::ConnectError {
                ...
                self.issue_event(GlobalCtxEvent::ConnectError(
                    url.to_string(),
                    format!("{ip_version:?}"),
                    error,
                ));
...
pub(crate) fn runtime_core_host_adapters(
    ...
    #[cfg(feature = "upnp")]
    {
        ...
        adapters.udp_hole_punch_events = Some(global_ctx.clone());
    }
    ...
    #[cfg(feature = "vpn-portal")]
    {
        adapters.vpn_portal_events = Some(global_ctx.clone());
    }
    #[cfg(feature = "smoltcp")]
    {
        adapters.gateway_events = Some(global_ctx);
    }
    adapters
}
