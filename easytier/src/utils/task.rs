...
{
    spawner: ManuallyDrop<TaskSpawner<F>>,
    f: Option<Task<F>>,
}
    ...
    <F as Future>::Output: Send + 'static,
{
        ...
        S: FnOnce(Task<F>) -> R + Send + 'static,
    {
        Self {
            f: Some(Box::pin(f)),
            spawner: ManuallyDrop::new(Box::new(|f| {
                spawner(f);
            })),
        }
    }
    ...
    <F as Future>::Output: Send + 'static,
{
    fn from(value: F) -> Self {
        Self::with_spawner(tokio::spawn, value)
    }
}
...
{
    fn drop(&mut self) {
        let spawner = unsafe { ManuallyDrop::take(&mut self.spawner) };
