...
use hickory_resolver::name_server::{
  GenericConnector, TokioConnectionProvider,
};
...
pub static ALLOW_USE_SYSTEM_DNS_RESOLVER: Lazy<AtomicBool> =
  Lazy::new(|| AtomicBool::new(true));

pub static RESOLVER: Lazy<
  Arc<Resolver<GenericConnector<TokioRuntimeProvider>>>,
> = Lazy::new(|| {
...
pub async fn resolve_txt_record(domain_name: &str) -> Result<String, Error> {
  ...
  let response = r.txt_lookup(domain_name).await.with_context(|| {
    format!("txt_lookup failed, domain_name: {}", domain_name)
  })?;

  let txt_record = response.iter().next().with_context(|| {
    format!("no txt record found, domain_name: {}", domain_name)
  })?;
...
pub async fn socket_addrs(
  ...
  match host {
    url::Host::Ipv4(ip) => {
      return Ok(vec![SocketAddr::new(std::net::IpAddr::V4(ip), port)])
    }
    url::Host::Ipv6(ip) => {
      return Ok(vec![SocketAddr::new(std::net::IpAddr::V6(ip), port)])
    }
    _ => {}
  }
  ...
  if ALLOW_USE_SYSTEM_DNS_RESOLVER.load(std::sync::atomic::Ordering::Relaxed) {
    ...
    match timeout(SYSTEM_DNS_LOOKUP_TIMEOUT, lookup_host(socket_addr)).await {
      Ok(Ok(a)) => {
        let a = a.collect();
      ...
      Ok(Err(e)) => {
        tracing::warn!(?e, "system dns lookup failed, fallback to hickory");
      }
      Err(_) => {
        tracing::warn!(
