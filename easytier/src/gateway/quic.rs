...
impl NatDstConnector for NatDstQUICConnector {
    ...
    async fn connect(&self, src: SocketAddr, nat_dst: SocketAddr) -> Result<Self::DstStream> {
        ...
        tracing::info!("quic nat dst: {:?}, dst peers: {:?}", nat_dst, dst_peers);

        if dst_peers.len() != 1 {
            return Err(anyhow::anyhow!("no dst peer found for nat dst: {}", nat_dst).into());
        }

        let Some(dst_ipv4) = peer_mgr.get_peer_map().get_peer_ipv4(dst_peers[0]).await else {
            return Err(anyhow::anyhow!("no ipv4 found for dst peer: {}", dst_peers[0]).into());
        };
        ...
        let connection = {
            ...
            endpoint
                .connect(SocketAddr::new(dst_ipv4.into(), 12345), "localhost")
                .unwrap()
                .await
                .with_context(|| {
                    format!(
...
impl QUICProxyDst {
    pub fn new(global_ctx: ArcGlobalCtx) -> Result<Self> {
        ...
        let (endpoint, _) = make_server_endpoint("0.0.0.0:12345".parse().unwrap())
