...
如果 stack feature 不能通过，说明完整 smoltcp stack 不能按当前形态进入 core。
此时必须回退为 runtime `SmolTcpStack` Adapter，不能为了迁移让 core 失去 wasm
编译目标。`proxy-packet` 仍应优先保留，因为它只依赖 `smoltcp::wire`，已经比
`pnet` 更适合 core packet 操作。
...
4. 迁移 TCP proxy core 和 stack seam
   ...
   - smoltcp dataplane 作为 core optional `proxy-smoltcp-stack` feature 迁入。
