...
- `TcpProxy` facade 可以继续暴露 `get_transport_type()`、`is_tcp_proxy_connection()`
  ...
  service/core snapshot，不能重新拥有 packet filter 或 NAT state。
...
`UdpProxyCore` 负责扣除 IPv4/UDP header，计算 UDP payload chunk。除最后一个
fragment 外，每个 IPv4 fragment payload 长度必须按 8-byte boundary 向下取整。
`UdpProxyCore` 持有 IPv4 ID allocator，负责把大 UDP response 按 MTU 切成 IPv4
...
`TcpProxyCore::set_local_port(local_port)`，然后才能注册 peer/NIC pipeline 或处理
任何 peer SYN packet。smoltcp 模式也同理，core `SmolTcpStack` 初始化完成并确定
fake local port 后，必须先写入 `TcpProxyCore` 再开放 packet pipeline。
...
core `UdpProxyService` 持有 `UdpProxyCore` 和 `PeerManagerCore`，注册
`PeerPacketFilter`，把 core action 分发给 peer egress 和 runtime socket
adapter。`UdpProxyCore` 保持纯 packet/state，不直接依赖 `PeerManagerCore`。
...
- core `TcpProxyService`：持有 `TcpProxyCore`、`TcpStack` 和 `PeerManagerCore`，
...
`TcpProxyCore` 保持纯 packet/state，不直接依赖 `PeerManagerCore`。依赖
`PeerManagerCore` 的层级是 core `TcpProxyService`，不是 runtime facade。
`TcpProxyCore` 通过 action 描述需要执行的动作：
...
- `TcpProxyCore` 的普通 NAT/packet rewrite 不依赖完整 smoltcp TCP stack。
...
5. 迁移 UDP proxy core
   - core 新增 `UdpProxyCore`、`UdpProxyService`、NAT table、runtime socket action。
...
6. 迁移 TCP proxy core 和 stack seam
   - core 新增 `TcpProxyCore`、`TcpProxyService` 和 `TcpStack` Interface。
   ...
   - kernel/smoltcp local port 必须在 pipeline 注册前写入 `TcpProxyCore`。
...
- TCP kernel/smoltcp local port 在 peer/NIC pipeline 注册前写入 `TcpProxyCore`。
