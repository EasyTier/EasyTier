...
- acceptor 被 core 消费并由 core 调用 `UdpHolePunchTunnelSink`，避免 runtime
  反向持有 peer graph。
- `create_port_bound_listener(port)` 表示 runtime 在指定本地 UDP port 上创建
  listener，不做 mapped address resolution，不建立 UPnP/NAT-PMP lease。它用于
  both-easy-sym 中发现 punched socket 后，按旧语义用同一个 local port 重新 bind
  并创建 listener/acceptor。它必须返回完整 listener，因为旧流程还会用这个新
  listener 的 socket 继续向 punched remote 发包。该 seam 只建模现有行为；不要在
  本轮迁移中改成复用 punched socket。
...
- runtime：提供 UDP socket、STUN/public addr、mapped listener、
  port-bound listener、connect-with-socket。
...
- runtime adapter 支持按指定本地端口创建 listener，保持当前
...
- 发现 punched socket 后，使用 punched socket 的 local port 调用
  `runtime.create_port_bound_listener(port)`，保持现有“重新 bind 指定端口”
  语义，并继续用新 listener 的 socket 向 punched remote 发包；不在本轮重构中
  改成复用 punched socket。
...
- 本轮只暴露 `create_port_bound_listener(port)`，保持现有
