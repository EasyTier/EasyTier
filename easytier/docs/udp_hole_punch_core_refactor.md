...
core 应该管理 listener 的复用，并在 accept 后调用注入的 tunnel sink；
runtime 只应该提供 listener 的 socket、mapped address、acceptor 和 lease。
...
- acceptor 被 core 消费并由 core 调用 `UdpHolePunchTunnelSink`，避免 runtime
  反向持有 peer graph。
- `create_port_bound_acceptor(port)` 表示 runtime 在指定本地 UDP port 上创建
  acceptor，不做 mapped address resolution，不建立 UPnP/NAT-PMP lease。它用于
  both-easy-sym 中发现 punched socket 后，按旧语义用同一个 local port 创建
  listener/acceptor。该 seam 只建模现有行为；不要在本轮迁移中改成复用
  punched socket。
...
- runtime：提供 UDP socket、STUN/public addr、mapped listener、
  port-bound acceptor、connect-with-socket。
...
- `RuntimeUdpPunchAcceptor` 支持按指定本地端口创建 acceptor，保持当前
  `UdpHolePunchListener::new_ext(..., false, Some(port))` 的语义。
...
- 发现 punched socket 后，使用 punched socket 的 local port 调用
  `runtime.create_port_bound_acceptor(port)`，保持现有“重新 bind 指定端口”
  语义，不在本轮重构中改成复用 punched socket。
...
- transaction id 匹配。
- listener selection。
- port-bound acceptor 创建时不触发 mapped address resolution 或 UPnP。
...
- both-easy-sym 的 port-bound acceptor 路径不做 public addr resolution，也不建立
  UPnP/NAT-PMP lease。
- 保留现有 upnp attempt 计数测试。

### both-easy-sym 端口绑定 acceptor

风险：为了让 runtime seam 更“干净”，把现有“按端口重新创建 acceptor”的语义
改成“复用 punched socket”，会把行为变更混入迁移。

处理：

- 本轮只暴露 `create_port_bound_acceptor(port)`，保持现有
  `UdpHolePunchListener::new_ext(..., false, Some(port))` 语义。
