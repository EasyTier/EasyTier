...
macro_rules! get_client_manager {
    () => {{
        let guard =
            tokio::time::timeout(std::time::Duration::from_millis(500), CLIENT_MANAGER.read())
                .await
                .map_err(|_| "get client manager timed out".to_string())?;
...
async fn init_rpc_connection(_app: AppHandle, url: Option<String>) -> Result<(), String> {
    let mut client_manager_guard = CLIENT_MANAGER
        .try_write()
