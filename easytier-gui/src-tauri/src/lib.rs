...
macro_rules! get_client_manager {
    () => {
        RwLockReadGuard::try_map(CLIENT_MANAGER.read().await, |cm| cm.as_ref())
            .map_err(|_| "RPC connection not initialized".to_string())
    };
    ($err_fn:expr) => {
        RwLockReadGuard::try_map(CLIENT_MANAGER.read().await, |cm| cm.as_ref()).map_err($err_fn)
    };
}
...
async fn init_rpc_connection(_app: AppHandle, url: Option<String>) -> Result<(), String> {
    ...
    let mut ring_rpc_server_guard = RING_RPC_SERVER.write().await;
...
mod manager {
    ...
    impl GUIStorage {
        ...
        pub(super) async fn load_configs(
            ...
            let client =
                get_client_manager!(|_| anyhow::anyhow!("RPC connection not initialized"))?
            ...
            for id in running_instances.inst_ids {
                self.enabled_networks.insert(id.into());
            }
            for id in enabled_networks {
                if let Ok(uuid) = id.parse() {
                    if !self.enabled_networks.contains(&uuid) {
                        let config = self.network_configs.get(&uuid).map(|i| i.value().1.clone());
                        if config.is_none() {
                            continue;
                        }
                        client
